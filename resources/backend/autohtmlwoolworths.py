...
<html lang="en">
	...
	<body>
		<div class="title">Woolworths</div>
	"""
	for j in range(4):
		out+="		<div class=\"row\">\n"
		for i in range(4):
			...
			out+=hi1
			out+="</p>\n"
			out+="				<a href=\"product" + str(coun+80) + ".html\"><img src=\"../images/" + str(coun+80) + ".jpg\" class=\"image\" /></a>\n"
			out+="				<p>" + str(hi2) + "</p>\n"
			out+="			</div>\n"
			coun+=1
		out+="		</div>\n"
	out+="		<div class=\"align\">\n"
	if k!=0:
		out+="			<a href=\"shop_woolworths" + str(k) + ".html\"><button>&larr;</button></a>\n"
	out+="			Page: " + str(k+1) + "\n"
	if k!=4:
		out+="			<a href=\"shop_woolworths" + str(k+2) + ".html\"><button>&rarr;</button></a>\n"
	out+="		</div>\n"
	out+="	</body>\n"
	out+="</html>"
	with open("woolworthshtml/shop_woolworths" + str(k+1) + ".html", "w") as data:
		data.write(out)
